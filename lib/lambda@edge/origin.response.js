...
const getRegion = (context) => {
    return context?.invokedFunctionArn?.split(":")[3] ?? "us-east-1";
};
...
const s3AccessPoints = {
    "transformation-template=webp-20230802-460": {
        "arn": context => "arn:aws:s3-object-lambda:" + getRegion(context) + ":" + getAccount(context) + ":accesspoint/imgtransf11af8f-" + getStackName(context)
    },
    "transformation-template=webp-20230802-920": {
        "arn": context =>  "arn:aws:s3-object-lambda:" + getRegion(context) + ":" + getAccount(context) + ":accesspoint/imgtransf7a03bd-" + getStackName(context)
    }
};
